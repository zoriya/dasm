...
{
	if (argc < 3 || (strcmp(argv[1], "-d") && strcmp(argv[1], "-i") && strcmp(argv[1], "-m"))) {
		printf("Usage:\n\tTo disassemble: %s -d <binary>\n\tTo interpret: %s -i <binary> or %s -m <binary> for debug mode\n", argv[0], argv[0], argv[0]);
		return 2;
	}
	...
	int ret;
	if (!strcmp(argv[1], "-d"))
		ret = dasm(binary, size);
	else
		ret = interpret(binary, size, argc - 2, argv + 2, !strcmp(argv[1], "-m"));
	free(binary);
	return ret;
}
